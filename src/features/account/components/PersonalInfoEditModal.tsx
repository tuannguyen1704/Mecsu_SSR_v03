...
import type { UserPersonalInfo } from "../types/account";

interface PersonalInfoEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: UserPersonalInfo;
  onSave: (data: UserPersonalInfo) => void;
}
...
export function PersonalInfoEditModal({
  ...
  const [formData, setFormData] = useState<UserPersonalInfo>(data);
  const [showToast, setShowToast] = useState(false);
  const [errors, setErrors] = useState<
    Partial<Record<keyof UserPersonalInfo, string>>
  >({});
  ...
  const handleChange = (field: keyof UserPersonalInfo, value: string) => {
  ...
  const validate = (): boolean => {
    const newErrors: Partial<Record<keyof UserPersonalInfo, string>> = {};
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-[#E5EAF2] bg-slate-50/50 px-6 py-4">
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#163F78]/10">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleChange(
                      "gender",
                      e.target.value as UserPersonalInfo["gender"]
                    )
                  }
